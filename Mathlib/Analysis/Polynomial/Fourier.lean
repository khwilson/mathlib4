...
We define a linear map from `ℂ[X]` to the `MeasureTheory.Lp` (with `p := 2`) space on the additive
...
/-- Linear map from `ℂ[X]` to `C(AddCircle (2 * π), ℂ)` that evaluates polynomials on the unit
